@extends('layouts.app')
@section('title', isset($item) ? 'Edit Stok Dapur' : 'Tambah Stok Dapur')
@section('content')
<div style="max-width:640px;">
    <div class="card">
        <div class="card-header"><span class="card-title">{{ isset($item) ? 'Edit Stok Dapur' : 'Tambah Stok Dapur' }}</span></div>
        <div class="card-body">
            <form method="POST" action="{{ isset($item) ? route('kitchen.update', $item->id) : route('kitchen.store') }}">
                @csrf
                ...
                <div style="display:flex;gap:8px;">
                    ...
                    <a href="{{ route('kitchen.index') }}" class="btn btn-secondary">Batal</a>
                </div>
            </form>
        </div>
    </div>
